...
For most consumers, the amount of financial support is relatively low as the distribution for expenditures is right skewed as evident by the mean > median.
...
#boxplot(dds.discr$age)
hist(dds.discr$age)

#numerical summaries
summary(dds.discr$age)
#addmargins(table(dds.discr$age.cohort , dds.discr$gender))
table(dds.discr$age.cohort)
```

The age distribution appears to be rightly skewed.
...
The consumers from this sample appear to be predominantly Hispanic and Non Hispanic White.
```{r}
0.376 + 0.401
```
...
#numerical summaries
prop.table(table(dds.discr$gender))

```
  
  Yes, there is a roughly 1:1 ratio between genders.
...
```
Not really any significant difference between genders.
...
The median for White's is greater than the 3rd quartile and about the same as the maximum non-outlier for Hispanics.
...
Whites receiving support tend to be disproportionately older than the Hispanics receiving support.
...
dds.hispanic <- dds.discr[dds.discr$ethnicity == "Hispanic", ]
dds.white <- dds.discr[dds.discr$ethnicity =="White not Hispanic", ]

#calculate mean expenditures by age cohort for Hispanics
hisp.means = tapply(dds.hispanic$expenditures, dds.hispanic$age.cohort, mean)
nonhisp.means = tapply(dds.white$expenditures, dds.white$age.cohort,
mean)
...
Not really. There appears to be a difference in funding between Hispanic and White consumers, but this may be influenced by the difference in age distribution between these two groups.
...
```{r}
#calculations
hispanic_props <- prop.table(table(dds.discr$age.cohort[dds.discr$ethnicity == "Hispanic"]))
weighted_hispanic_funds <- hispanic_props * hisp.means
sum(weighted_hispanic_funds)

white_props <- prop.table(table(dds.discr$age.cohort[dds.discr$ethnicity == "White not Hispanic"]))
weighted_white_funds <- white_props * nonhisp.means
sum(weighted_white_funds)
```


The paradox is explainable by the fact that there is a greater proportion of younger Hispanics than the proportion of younger aged consumers in the White subset. 

Younger consumers tend to receive less funds.