...
It is important to sample without replacement to avoid picking the same genes.

Selecting the same genes would decrease the number of factors in the training.
...
gene.index.set contains the 100 indices of genes that are included as the column names in the gene.matrix.sample sample of the golub.train gene.matrix.
...
```{r}
selected.genes <- colnames(gene.matrix.sample)[1:5]
selected.genes
```
...
Unimodal with slight right skew.
...
We use AML as the variable condition because there is only a single group of AML patients, but ALL can be classified into ALL-B or ALL-T, which we group together into ALL for this study.
...
42 patients have ALL and 20 patients have AML.
